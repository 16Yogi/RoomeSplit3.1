...
import { CheckCircle2, Circle, ArrowRight, Info, CreditCard, Send, TrendingUp, Calendar, ChevronDown, ChevronUp, Filter, X, User, XCircle, Users } from 'lucide-react';
...
const SettlementMatrix: React.FC<SettlementMatrixProps> = ({ roommates, expenses, sharedPurchases = [], paidRoommateIds, onTogglePaid, onAddExpense, fixedCosts }) => {
  ...
  const calculateSettlements = (expenseList: Expense[], purchaseList: SharedPurchase[] = []) => {
    ...
    purchaseList.forEach(purchase => {
      if (purchase.buyer !== purchase.payer) {
        // Scenario: Buyer bought item, Payer paid for it
        // Example: Mohan bought facewash (for himself), Ramu paid ₹30
        // Result: Mohan owes Ramu ₹30 (only the paid amount, not split)
        
        // This is a direct debt scenario, not a shared expense
        // Don't add to spending - it's not split among roommates
        // Just record the direct debt: buyer owes payer the full amount
        ...
        // Buyer and payer are same - treat as regular expense
        // They bought and paid themselves, so add to their spending
        spendingMap.set(purchase.buyer, (spendingMap.get(purchase.buyer) || 0) + purchase.amount);
      }
    });

    return { purchaseSpending: spendingMap, directPayments: paymentsMap, sharedPurchaseDebts };
  };
  ...
  const filteredPurchases = useMemo(() => {
    ...
    return sharedPurchases.filter(purchase => {
      return isDateInRange(purchase.date, startDate || null, endDate || null);
    });
  ...
  const { purchaseSpending, directPayments, sharedPurchaseDebts } = useMemo(() => {
  ...
  const getDebt = (rowName: string, colName: string, spending: Map<string, number>, payments: Map<string, number>, sharedPurchaseDebts?: Map<string, number>) => {
    ...
    const sharedDebtKey = `${rowName}-${colName}`;
    const sharedDebt = sharedPurchaseDebts?.get(sharedDebtKey) || 0;

    return baseDebt - iPaidJ + jPaidI + sharedDebt;
  };

  const getSettlementInstructions = (spending: Map<string, number>, payments: Map<string, number>, sharedDebts?: Map<string, number>) => {
    const inst: { from: string; to: string; amount: number }[] = [];
    ...
    for (let i = 0; i < roommates.length; i++) {
      for (let j = 0; j < roommates.length; j++) {
        ...
        if (debt > 0.01) {
          inst.push({ from: roommates[i].name, to: roommates[j].name, amount: debt });
        }
      }
    }
    return inst;
  ...
  const settlementInstructions = useMemo(() => {
    return getSettlementInstructions(purchaseSpending, directPayments, sharedPurchaseDebts);
  }, [purchaseSpending, directPayments, sharedPurchaseDebts, roommates]);
  ...
  const renderMatrix = (spending: Map<string, number>, payments: Map<string, number>, title?: string, sharedDebts?: Map<string, number>) => {
          ...
          <table className="w-full text-sm border-collapse">
            ...
            <tbody className="divide-y divide-gray-100">
              {roommates.map(rowUser => {
                const spent = spending.get(rowUser.name) || 0;
  ...
  return (
    <div className="flex flex-col bg-white space-y-6">
      ...
      {/* Current View Matrix */}
      {renderMatrix(purchaseSpending, directPayments, getDateRangeText(), sharedPurchaseDebts)}
                ...
                <div key={month} className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                  ...
                  {isExpanded && (
                    <div className="p-4 border-t border-gray-100 space-y-4">
                      {renderMatrix(monthData.purchaseSpending, monthData.directPayments, undefined, monthData.sharedPurchaseDebts)}
